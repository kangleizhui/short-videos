...
用法:
  ...
  curl -s "https://spqsy.kcucu.com/api/parse.php?key=KEY&url=URL" | python3 synthesize_live.py
...
def main():
    ...
    if sys.argv[1] == '--url':
        ...
        api = f"https://spqsy.kcucu.com/api/parse.php?key={key}&url={urllib.parse.quote(url)}"
        print(f"🔍 解析: {url}")
